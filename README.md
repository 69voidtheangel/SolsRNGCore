...
SolsRNGCore includes a built-in Roblox item automation system designed for
long-running Sol's RNG sessions.

Automation can be configured with:

- Strange Controller
- Biome Randomizer
- Per-item cooldowns
- A shared coordinate configuration
- Inventory, item, search, slot, quantity, use, and close-inventory actions
- Game-window detection and restoration
- Automation testing
- Start / Stop controls

The automation system uses the same coordinate set for all configured items,
so the interface does not require separate coordinate layouts for each item.

## ⚡ Automation Priority

When Anti-AFK and Automation are both running, Anti-AFK has the highest
execution priority.
...
Automation items never interrupt one another. A currently running automation
cycle finishes its complete sequence before another task takes control.

When Anti-AFK becomes due, it receives the next available execution turn
before another automation item starts.

This shared priority system prevents Anti-AFK and Roblox inventory automation
from fighting over focus or input.

## 🖥️ KDE Wayland

KDE Wayland is supported using compositor-aware window management.

SolsRNGCore can use:

- `wdotool` for KDE Wayland window focus, restoration, mouse movement, and
  clicking
- `ydotool` for keyboard input

This allows Automation and Anti-AFK to work correctly across multi-monitor
KDE Wayland setups.

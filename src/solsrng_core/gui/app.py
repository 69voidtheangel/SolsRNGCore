...
from solsrng_core.platform.linux.environment import detect_environment
...
class MainWindow(QMainWindow):
    ...
    def _settings_page(self) -> QWidget:
        ...
        if kde_wayland:
            # KDE Wayland uses the tested Sober-focus + Space path only.
            current_method = "space"
            self.config.anti_afk.method = "space"
        ...
        environment_text = QLabel(
            "\n".join(
                [
                    f"Distribution: {distro.name}{distro_version}",
                    ...
                    (
                        ...
                        + (
                            "Available"
                            if __import__("shutil").which("wdotool")
                            else "Unavailable"
                        )
                    ),
                ]
    ...
    def _collect(self):
        ...
        if hasattr(self, "afk_method"):
            ...
            if kde_wayland:
                method = "space"
            elif method not in {"space", "alt_tab"}:
                method = "space"
